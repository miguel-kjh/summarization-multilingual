...
for i in "${!languages[@]}"; do
    ...
    python3 baseline.py --dataset "$dataset" --method "lsa"
    python3 model_evaluate.py --model_name_or_path "models/baseline/$lang/lsa/qwen2.5:0.5b" --dataset "$dataset"

    if [ "$lang" != "canario" ]; then
        python3 baseline.py --dataset "$dataset" --method "lsa" --truncate True
        python3 model_evaluate.py --model_name_or_path "models/baseline/$lang/lsa/qwen2.5:0.5b" --dataset "$dataset" --method "truncate" --use_openai False
    fi

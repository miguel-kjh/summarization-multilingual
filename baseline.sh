...
for i in "${!languages[@]}"; do
    ...
    python3 model_evaluate.py --model_name_or_path "models/baseline/$lang/textranking/qwen2.5:0.5b" --dataset "$dataset"

    #python3 baseline.py --dataset "$dataset" --method "ghic" --truncate True
    python3 model_evaluate.py --model_name_or_path "models/baseline/$lang/ghic/" --dataset "$dataset"

    #python3 baseline.py --dataset "$dataset" --method "extractive" --model_name "bert-base-multilingual-cased" --truncate True
    python3 model_evaluate.py --model_name_or_path "models/baseline/$lang/extractive/bert-base-multilingual-cased" --dataset "$dataset"

    # Extra para inglés
    if [ "$lang" == "english" ]; then
        #python3 baseline.py --dataset "$dataset" --method "extractive" --model_name "nlpaueb/legal-bert-base-uncased"
        python3 model_evaluate.py --model_name_or_path "models/baseline/$lang/extractive/nlpaueb/legal-bert-base-uncased" --dataset "$dataset" 
    fi
    ...
    if [ "$lang" != "canario" ]; then
        ...
        python3 model_evaluate.py --model_name_or_path "models/baseline/$lang/openai" --dataset "$dataset"
    fi
    ...
    for model_name in "${models[@]}"; do
        ...
        python3 model_evaluate.py --model_name_or_path "models/baseline/$lang/ollama/$model_name" --dataset "$dataset"
